...
class Trainer(transformers.Trainer):
    def create_optimizer_and_scheduler(self, num_training_steps: int):
        if self.optimizer is None:
            ...
            for n, p in self.model.named_parameters():
                if self.args.fix_except_prompt:
                    if 'prompt_tower' not in n:
                        params[n] = p
                else:
    ...
    def prepare_for_soft_labels(self, eval_dataset: Optional[Dataset] = None) -> Dict[str, float]:
        if eval_dataset is not None and not isinstance(eval_dataset, collections.abc.Sized):
            raise ValueError("eval_dataset must implement __len__")
        ...
        output = torch.tensor(output.predictions).cuda()
        soft_label_embeddings = torch.mean(output, 0)
